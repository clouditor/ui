			...
			<li class="{stepIdx !== steps.length - 1 ? 'pb-10' : ''} relative">
				{#if stepIdx < current}
					{#if stepIdx !== steps.length - 1}
						<div
							class="bg-clouditor absolute top-4 left-4 mt-0.5 -ml-px h-full w-0.5"
							aria-hidden="true"
						></div>
					{/if}
					<a href={step.href} class="group relative flex items-start">
						<span class="flex h-9 items-center">
							<span
								class="bg-clouditor group-hover:bg-clouditor-light relative z-10 flex h-8 w-8 items-center justify-center rounded-full"
							>
				...
				{:else if stepIdx == current}
					...
					<span class="group relative flex items-start" aria-current="step">
						<span class="flex h-9 items-center" aria-hidden="true">
							<span
								class="border-clouditor relative z-10 flex h-8 w-8 items-center justify-center rounded-full border-2 bg-white"
							>
								<span class="bg-clouditor h-2.5 w-2.5 rounded-full"></span>
							</span>
						</span>
						<span class="ml-4 flex min-w-0 flex-col">
							<span class="text-clouditor text-sm font-medium">{step.name}</span>
							<span class="text-sm text-gray-500">{step.description}</span>
						</span>
					</span>
